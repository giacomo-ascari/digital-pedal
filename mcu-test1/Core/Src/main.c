...
void HAL_I2S_RxCpltCallback(I2S_HandleTypeDef *hi2s)
{
	...
	if (hi2s->Instance != SPI2) {
		if (HAL_GPIO_ReadPin(Btn2_GPIO_Port, Btn2_Pin) == GPIO_PIN_RESET) {
			float val_f = wave_gen('s', counter, 880.0F) * 8000.F;
			int16_t val_i = (int16_t)val_f;
			HAL_DAC_SetValue(&hdac, DAC_CHANNEL_1, DAC_ALIGN_12B_R, val_i);
			counter++;
		} else {
			int16_t *buff[1];
			HAL_I2S_Receive_DMA(hi2s, (int16_t *)buff, 1);
			HAL_DAC_SetValue(&hdac, DAC_CHANNEL_1, DAC_ALIGN_12B_R, buff[0]);
		}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	 ...
	 */
	if (htim->Instance == TIM2 && HAL_GPIO_ReadPin(Btn3_GPIO_Port, Btn3_Pin) == GPIO_PIN_RESET) {
		float val_f = wave_gen('s', counter, 880.0F) * 8000.F;
		int16_t val_i = (int16_t)val_f;
		HAL_DAC_SetValue(&hdac, DAC_CHANNEL_1, DAC_ALIGN_12B_R, val_i);
		counter++;
