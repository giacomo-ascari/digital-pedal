...
int main(int argc, char** argv)
{
    ...
    pedalboard_t *p_pedalboard = pedalboard_init();
    for (u_int32_t i = 3; i < argc; i++) {
        pedalboard_add(p_pedalboard, pedal_type_parse(argv[i]));
    }
    ...
    while(!feof(raw_file))
    {
        ...
        out_samples[i] = pedalboard_process(p_pedalboard, raw_samples[i]);
