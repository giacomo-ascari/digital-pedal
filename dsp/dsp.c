...
abstract_pedal_t *abstract_pedal_init(enum pedal_types type) {
    ...
    if (p_pd->type == BYPASS) {
        ...
        p_pd->pedal = bitcrusher_rs_pedal_init();
    } else if (type == TREMOLO) {
        p_pd->pedal = tremolo_pedal_init();
    } else if (type == LPF) {
        p_pd->pedal = low_pass_filter_pedal_init();
    }
    return p_pd;
}
...
void abstract_pedal_destroy(abstract_pedal_t *p_pd) {
    if (p_pd->type == BYPASS) {
        ...
        bitcrusher_rs_pedal_destroy(p_pd->pedal);
    } else if (p_pd->type == TREMOLO) {
        tremolo_pedal_destroy(p_pd->pedal);
    } else if (p_pd->type == LPF) {
        low_pass_filter_pedal_destroy(p_pd->pedal);
    }
    free(p_pd);
}
...
pedalboard_t *pedalboard_init() {
    pedalboard_t *p_pb = malloc(sizeof(pedalboard_t));
...
void pedalboard_add(pedalboard_t *p_pb, enum pedal_types type) {
    abstract_pedal_t *p_pd = abstract_pedal_init(type);
    p_pb->pedals[p_pb->active_pedals] = p_pd;
    p_pb->active_pedals++;
}
...
float pedalboard_process(pedalboard_t *p_pb, float in) {
    float out = in;
    for (u_int32_t i = 0; i < p_pb->active_pedals; i++) {
        out = abstract_pedal_process(p_pb->pedals[i], out);
    }
    return out;
}
