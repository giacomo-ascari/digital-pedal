...
int main(void)
{
	...
	while (1)
	{
		...
		for (int i = 0; i < 6; i++) {
			if (i == selected % 6) {
				HAL_GPIO_WritePin(led_port[i], led_pins[i], GPIO_PIN_SET);
			} else {
				HAL_GPIO_WritePin(led_port[i], led_pins[i], GPIO_PIN_RESET);
			}
		}
		selected++;
		HAL_Delay(250);
...
static void MX_GPIO_Init(void)
{
	...
	HAL_GPIO_WritePin(GPIOA, LD6_Pin|LD5_Pin|LD4_Pin, GPIO_PIN_RESET);

	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOB, LD3_Pin|LD2_Pin|LD1_Pin, GPIO_PIN_RESET);

	/*Configure GPIO pins : LD6_Pin LD5_Pin LD4_Pin */
	GPIO_InitStruct.Pin = LD6_Pin|LD5_Pin|LD4_Pin;
