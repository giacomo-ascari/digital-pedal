...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	if (hre1.pinA == GPIO_Pin) RE_Process(&hre1);
	if (hre2.pinA == GPIO_Pin) RE_Process(&hre2);
}
...
void command_callback(Command command) {
	...
	if (command.header > 0 && command.header <= 6 && command.subheader > 0 && command.subheader <= 16) {
    ...
    /* USER CODE BEGIN 3 */
		...
		if (new_page != hmenu.selected_page) {
			...
			if (hmenu.selected_page == PLOT) {
				...
				if (RE_Process(&hre1)) {
					hmenu.edit_index1 += RE_GetCount(&hre1) - enc1_old;
					enc1_old = RE_GetCount(&hre1);
					Menu_UpdatePage(&hmenu);
				}
				if (RE_Process(&hre2)) {
					hmenu.edit_index2 += RE_GetCount(&hre2) - enc2_old;
					enc2_old = RE_GetCount(&hre2);
					Menu_UpdatePage(&hmenu);
				}
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : EPD_BUSY_Pin ENC1B_Pin */
  GPIO_InitStruct.Pin = EPD_BUSY_Pin|ENC1B_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
